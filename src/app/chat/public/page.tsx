...
import { ref, push, serverTimestamp, query, limitToLast, remove, update, get } from 'firebase/database';
...
export default function PublicChatPage() {
  ...
  const posts = useMemo(() => {
    ...
    return Object.entries(postsData)
      ...
      .filter((p: any) => p.type !== 'dispute') // فلترة النزاعات لعرضها في المحاكمة فقط
  ...
  const handleSendPost = async (e: React.FormEvent) => {
    ...
    if (!isPremium && remainingPosts <= 0) {
      toast({ variant: "destructive", title: "وصلت للحد اليومي", description: "يسمح ببوستين فقط للأعضاء العاديين. اشترك في بريميوم للنشر بلا حدود! 👑" });
      return;
    }
    ...
    <div className="min-h-screen bg-background md:pr-72 pb-40" dir="rtl">
      ...
      <div className="app-container py-6 space-y-6">
            ...
            <form onSubmit={handleSendPost} className="space-y-4">
              <div className="flex items-start gap-3">
                <Link href="/profile">
                  <div className="w-10 h-10 bg-secondary rounded-full flex items-center justify-center text-xl overflow-hidden border border-border">
                    {userData?.avatar?.startsWith('data:image') || userData?.avatar?.startsWith('http') ? <img src={userData.avatar} className="w-full h-full object-cover" alt="Me" /> : <span>{userData?.avatar || "🐱"}</span>}
                  </div>
                </Link>
                <textarea 
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  <Button type="button" onClick={() => fileInputRef.current?.click()} disabled={isCompressing} variant="ghost" size="icon" className="h-12 w-12 rounded-xl text-primary bg-secondary/50">
        ...
        <div className="space-y-4 mx-2">
          {posts.length === 0 ? <div className="text-center py-20 opacity-20 font-black text-xl">كن أول من ينشر في المجتمع! 🐱✨</div> : posts.map((post) => (
            <Card key={post.id} className="rounded-[2rem] shadow-md border border-border bg-card overflow-hidden">
              <div className="p-5 pb-2 flex flex-row items-center justify-between">
                <div className="flex items-center gap-3">
                  <Link href={`/user/${post.userId}`} onClick={() => playSound('click')}>
                    <div className="w-11 h-11 bg-white rounded-full flex items-center justify-center text-xl border border-border overflow-hidden hover:scale-105 transition-transform">
                      {post.userAvatar?.startsWith('data:image') || post.userAvatar?.startsWith('http') ? <img src={post.userAvatar} className="w-full h-full object-cover" alt={post.userName} /> : <span>{post.userAvatar || "🐱"}</span>}
                    </div>
                  </Link>
                  <div className="text-right">
                    <div className="flex items-center gap-1">
                      <Link href={`/user/${post.userId}`} className="font-black text-primary text-sm hover:underline">{post.userName}</Link>
                ...
                {(post.userId === user?.uid || isAdmin) && (
                  <AlertDialog>
                    <AlertDialogTrigger asChild><Button variant="ghost" size="icon" className="rounded-full text-destructive/40 hover:text-destructive"><Trash2 size={16} /></Button></AlertDialogTrigger>
              ...
              <CardContent className="p-5 pt-2 space-y-4">
                {post.image && <div className="rounded-2xl overflow-hidden border border-border shadow-sm mb-2"><img src={post.image} className="w-full h-auto max-h-96 object-contain bg-secondary/20" alt="Post" /></div>}
