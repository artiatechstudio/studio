...
import { Globe, Send, Trash2, Heart, Clock, Crown, Sparkles, Infinity, User, Camera, X, Loader2 } from 'lucide-react';
...
export default function PublicChatPage() {
  ...
  const posts = useMemo(() => {
    ...
    return Object.entries(postsData)
      ...
      .filter((p: any) => p.type !== 'dispute') // استبعاد النزاعات من جدار المنشورات العادية
            ...
            <form onSubmit={handleSendPost} className="space-y-4">
              <div className="flex items-start gap-3">
                ...
                <div className="flex-1 space-y-2">
                  ...
                  />
                  {postImage && (
                    <div className="relative w-32 h-32 rounded-xl overflow-hidden border-2 border-primary shadow-md">
                      <img src={postImage} className="w-full h-full object-cover" alt="Selected" />
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <Button type="button" onClick={() => fileInputRef.current?.click()} size="icon" variant="ghost" className="h-12 w-12 rounded-xl text-primary bg-secondary/50 hover:bg-primary/10" disabled={isCompressing}>
                ...
                <Button 
                  type="submit" 
                  disabled={(!postText.trim() && !postImage) || isSending || (!isPremium && remainingPosts <= 0)}
                  className="h-12 px-8 rounded-xl bg-primary hover:bg-primary/90 shadow-lg font-black gap-2"
                >
                  {isSending ? <Loader2 className="animate-spin" size={16}/> : "نشر الآن"} <Send size={16} className="rotate-180" />
                </Button>
              </div>
            </form>
          </CardContent>
          ...
          {posts.length === 0 ? (
            ...
            <Card key={post.id} className="rounded-[2rem] shadow-md border border-border bg-card overflow-hidden">
              ...
              <CardContent className="p-5 pt-2 space-y-4">
                ...
                {post.image && (
                  <div className="w-full rounded-2xl overflow-hidden border-2 border-secondary bg-black/5 flex justify-center">
                    <img src={post.image} className="max-h-[400px] w-auto object-contain" alt="Post" />
                  </div>
                )}
