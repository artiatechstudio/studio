  ...
  return (
    <div className="min-h-screen bg-background pb-32 md:pr-72" dir="rtl">
      ...
      <div className="app-container py-6 md:py-10 space-y-6">
        ...
        <header className="flex items-center justify-between bg-card p-4 rounded-[2rem] shadow-lg border border-border sticky top-4 z-30 mx-1">
          <div className="flex items-center gap-3 overflow-hidden">
            <div className="w-12 h-12 bg-primary rounded-xl flex items-center justify-center text-white text-2xl shrink-0 shadow-md">
              {profile.avatar || "🐱"}
            </div>
            <div className="flex flex-col text-right overflow-hidden">
              <p className="text-[10px] font-black text-muted-foreground">أهلاً بك</p>
              <p className="text-sm font-black text-primary leading-tight truncate max-w-[100px] sm:max-w-none">{profile.name}</p>
            </div>
          </div>
          
          <div className="flex items-center gap-2 shrink-0">
            <Link href="/streak" onClick={() => playSound('click')}>
              <div className="flex items-center gap-1.5 bg-orange-100 dark:bg-orange-900/30 px-3 py-1.5 rounded-full border border-orange-200 dark:border-orange-800 shadow-sm">
            ...
            <div className="flex items-center gap-1.5 bg-yellow-100 dark:bg-yellow-900/30 px-3 py-1.5 rounded-full border border-yellow-200 dark:border-yellow-800 shadow-sm">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mx-1">
          <Link href="/profile" className="block">
            <Card className="p-4 rounded-[2rem] shadow-md border border-border flex items-center gap-4 bg-card hover:scale-[1.01] transition-transform h-full">
              <div className="w-12 h-12 bg-green-100 dark:bg-green-900/30 rounded-xl flex items-center justify-center text-green-600 shrink-0 shadow-inner">
                <HeartPulse size={28} />
              </div>
              <div className="overflow-hidden">
                <p className="text-[10px] font-black text-muted-foreground uppercase mb-0.5">مؤشر الجسم (BMI)</p>
                <div className="flex items-center gap-2">
                  <span className="text-xl font-black text-primary">{bmiInfo?.value || '--'}</span>
                  <span className={cn("text-[10px] font-black px-2 py-0.5 rounded-full bg-secondary shadow-sm", bmiInfo?.color)}>
                    {bmiInfo?.status || '--'}
                  </span>
                </div>
              </div>
          ...
          <Link href="/streak" className="block">
            <Card className="p-4 rounded-[2rem] shadow-md border border-border flex items-center gap-4 bg-card hover:scale-[1.01] transition-transform h-full">
              <div className="w-12 h-12 bg-accent/10 rounded-xl flex items-center justify-center text-accent shrink-0 shadow-inner">
                <Activity size={28} />
              </div>
              <div className="flex-1 overflow-hidden">
                <p className="text-[10px] font-black text-muted-foreground uppercase mb-0.5">الإنجاز الكلي</p>
                <div className="flex items-center gap-3">
                  <span className="text-xl font-black text-primary">{progressPercent}%</span>
                  <div className="flex-1 bg-secondary h-2 rounded-full overflow-hidden hidden sm:block shadow-inner">
        ...
        <section className="bg-primary/5 rounded-[2.5rem] p-5 border border-primary/10 mx-1 shadow-inner cursor-pointer" onClick={() => router.push('/streak')}>
          <Mascot />
        </section>
