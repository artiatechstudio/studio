  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="max-w-5xl mx-auto p-6 md:p-12 space-y-10 pb-32">
        {/* رأس الصفحة */}
        <header className="flex flex-col md:flex-row items-center gap-8 bg-card p-8 md:p-12 rounded-[3rem] shadow-2xl border border-border">
          <div className="relative group">
            ...
            <div className="absolute -bottom-2 -right-2 bg-accent text-white p-3 rounded-2xl shadow-lg border-4 border-white dark:border-card">
          ...
          <div className="shrink-0 w-full md:w-auto flex flex-col gap-3">
            ...
            <Button onClick={handleLogout} variant="ghost" className="w-full md:w-auto rounded-2xl text-destructive hover:bg-destructive/10 px-8 py-6 text-lg font-black">
              ...
              <CardContent className="p-8 flex flex-wrap gap-4">
                {userData.badges?.length > 0 ? userData.badges.map((badge: string, i: number) => (
                  <div key={i} className="bg-secondary/40 px-6 py-4 rounded-[1.5rem] flex items-center gap-4 group hover:bg-primary hover:text-white transition-all cursor-default shadow-sm">
                    <div className="w-12 h-12 bg-card rounded-xl flex items-center justify-center text-primary group-hover:bg-card group-hover:text-primary shadow-sm">
                      <Trophy size={24} />
                    </div>
