...
for (let i = 1; i <= 120; i++) {
  ...
  const titleBase = masterBaseTitles[type][i % 10];
  ...
  ADDITIONAL_MASTER_CHALLENGES.push({
    ...
    time,
    title: `ماستر ${type} ${i}: ${titleBase}`,
    description: `هذا تحدي عشوائي متقدم لرفع مستوى مهاراتك في ${type}. المطلوب هو الاستمرارية والتركيز لمدة ${time} دقيقة.`,
